...
const form = reactive({ email: '', password: '', username: '', notes: '', concurrency: 1, role: 'user' as string, rpm_limit: 0, customAttributes: {} as UserAttributeValuesMap })

watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', notes: u.notes || '', concurrency: u.concurrency, role: u.role || 'user', rpm_limit: u.rpm_limit ?? 0, customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    const data: any = { email: form.email, username: form.username, notes: form.notes, concurrency: form.concurrency, role: form.role, rpm_limit: form.rpm_limit }
