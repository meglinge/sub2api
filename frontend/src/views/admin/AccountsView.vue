<template>
  <AppLayout>
    ...
    <AccountChatTestModal :show="showChatTest" :account="chatTestAcc" @close="closeChatTestModal" />
    <AccountStatsModal :show="showStats" :account="statsAcc" @close="closeStatsModal" />
    <ScheduledTestsPanel :show="showSchedulePanel" :account-id="scheduleAcc?.id ?? null" :model-options="scheduleModelOptions" @close="closeSchedulePanel" />
    <AccountActionMenu :show="menu.show" :account="menu.acc" :position="menu.pos" @close="menu.show = false" @test="handleTest" @test-chat="handleChatTest" @stats="handleViewStats" @schedule="handleSchedule" @reauth="handleReAuth" @refresh-token="handleRefresh" @recover-state="handleRecoverState" @reset-quota="handleResetQuota" @set-privacy="handleSetPrivacy" @create-spark-shadow="handleCreateSparkShadow" />
...
import AccountChatTestModal from '@/components/admin/account/AccountChatTestModal.vue'
...
const chatTestAcc = ref<Account | null>(null)
...
const loadSavedColumns = () => {
  try {
    ...
    if (saved) {
      ...
      // fork 迁移：一次性为老用户取消隐藏 today_stats（恢复默认显示）。
      ...
      // upstream 迁移：一次性为老用户隐藏 scheduler_score（避免开启重后端打分）。
...
const closeChatTestModal = () => { showChatTest.value = false; chatTestAcc.value = null }
const closeStatsModal = () => { showStats.value = false; statsAcc.value = null }
const closeReAuthModal = () => { showReAuth.value = false; reAuthAcc.value = null }
const handleTest = (a: Account) => { testingAcc.value = a; showTest.value = true }
const handleChatTest = (a: Account) => { chatTestAcc.value = a; showChatTest.value = true }
