...
func normalizeOpenAICompactRequestBody(body []byte) ([]byte, bool, error) {
	...
	// Responses Lite compact rejects parallel_tool_calls=true. When tools exist,
	// force false even if the client left it true. Without tools the field is
	// dropped above — re-adding false would fight that contract.
	if openAIRequestBodyHasTools(normalized) && (!gjson.GetBytes(normalized, "parallel_tool_calls").Exists() || gjson.GetBytes(normalized, "parallel_tool_calls").Bool()) {
