...
// openAIPassthroughRequestPath 返回入站请求的原始路径（去除首尾空白）。
...
// openAIResponsesRequestPathSuffixFromPath 从请求路径字符串提取 /responses 之后的子路径后缀，
// 便于在没有 gin.Context 的场景（如 403 探测 / 账号测试）复用同一逻辑。
...
// buildOpenAIResponsesURLForRequestPath 在 responses 基础 URL 上拼接请求路径后缀（如 /compact）。
