	...
	if !req.StickyWeighted {
		...
		if escapedSticky {
			...
			if s != nil && s.service != nil {
				...
				if accountID <= 0 {
					accountID, _ = s.service.getStickySessionAccountID(ctx, req.GroupID, req.SessionHash)
				}
				// Avoid reselecting the escaped account while retaining the
				// load balancer's fallback when it is the pool's last account.
				rememberOpenAIStickySkip(req.SessionHash, accountID)
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !account.IsSchedulableForRequest(AllowControlPlaneSchedule(ctx)) {
			if account.IsExcludedFromSchedule() {
				filterStats.exclude("exclude_from_schedule")
			} else {
