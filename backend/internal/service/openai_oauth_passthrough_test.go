...
func TestOpenAIGatewayService_APIKeyPassthrough_ForceCodexByGroupOverridesHeaders(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-force-codex"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":1,"output_tokens":1,"input_tokens_details":{"cached_tokens":0}}}`)),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	...
	account := &Account{
		...
		Type:           AccountTypeAPIKey,
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "sk-api-key", "base_url": "https://api.openai.com", "user_agent": "custom-account-agent/1.0"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	require.Equal(t, openAICodexOriginator, upstream.lastReq.Header.Get("originator"))
}

func TestOpenAIGatewayService_APIKeyPassthrough_CompactUsesCompactEndpoint(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.2-codex","stream":false,"store":true,"input":[{"type":"text","text":"compact me"}]}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-compact"}},
	...
	account := &Account{
		...
		Type:           AccountTypeAPIKey,
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "sk-api-key", "base_url": "https://api.openai.com/v1"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
...
func TestOpenAIGatewayService_OAuthCompact_NonPassthroughSanitizesStoreAndStream(t *testing.T) {
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-compact-non-passthrough"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":6,"output_tokens":2,"input_tokens_details":{"cached_tokens":1}}}`)),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	...
	account := &Account{
		ID:             459,
		Name:           "oauth-non-passthrough-compact",
		Platform:       PlatformOpenAI,
		Type:           AccountTypeOAuth,
		Concurrency:    1,
		Credentials:    map[string]any{"access_token": "oauth-token", "chatgpt_account_id": "chatgpt-acc"},
		Extra:          map[string]any{}, // passthrough disabled
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	}
...
func TestNormalizeOpenAIPassthroughOAuthBodyByRequestPath(t *testing.T) {
	...
	t.Run("responses_forces_store_false_and_stream_true", func(t *testing.T) {
		...
		normalized, changed, err := normalizeOpenAIPassthroughOAuthBody(input, false)
		require.NoError(t, err)
	...
	t.Run("compact_removes_store_and_stream", func(t *testing.T) {
		...
		normalized, changed, err := normalizeOpenAIPassthroughOAuthBody(input, true)
		require.NoError(t, err)
		require.True(t, changed)
