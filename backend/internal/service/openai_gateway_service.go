...
const (
	...
	codexCLIUserAgent      = openAICodexOriginator + "/" + openAICodexStableVersion
	...
	codexCLIVersion                    = openAICodexStableVersion
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}) *Account {
	...
	for i := range accounts {
		...
		windowEval := evaluateOpenAIUsageWindowWithConfig(fresh, now, windowCfg)
		s.maybeRefreshOpenAIUsageWindowAsync(fresh, windowEval)
		if windowEval.State == openAIUsageWindowStateRed {
			continue
		}
		if windowEval.State == openAIUsageWindowStateYellow {
			if degraded == nil || s.isBetterAccount(fresh, degraded) {
				degraded = fresh
			}
			continue
		}
