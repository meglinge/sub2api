...
func TestIsUpstreamModelNotFoundError(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "400 model_not_found code and unknown provider message",
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"error":{"message":"unknown provider for model gpt-5.6-sol","type":"invalid_request_error","param":"model","code":"model_not_found"}}`),
			want:       true,
		},
		{
		...
		},
		{
			...
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"error":{"message":"unknown model gpt-5.6-sol"}}`),
			want:       true,
		},
		{
			name:       "400 bare not found without strong model rejection does not match",
			statusCode: http.StatusBadRequest,
			body:       []byte(`{"error":{"message":"resource not found for model context"}}`),
			want:       false,
		},
		{
		...
		{
			name:       "500 with model not found does not match",
