...
func TestOpenAIPoolModeTempRule_StopsSameAccountRetryAndIsolatesBlockToModel(t *testing.T) {
	...
	// Use a model-specific rule match; generic service unavailability is
	// request-scoped capacity shedding in this fork and must not penalize health.
	body := []byte(`{"error":{"message":"Model temporarily unavailable"}}`)
	resp := &http.Response{
