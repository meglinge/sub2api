...
func appendGrokFreeCacheNativeToolsWithPolicy(body []byte, allowPureClientTools, allowFunctionSearch bool) ([]byte, error) {
	...
	if !allowPureClientTools && !allowFunctionSearch && !hasSearchIntent {
		return body, nil
	}
	...
	for _, tool := range items {
		...
		case "function":
			...
			if (name == "web_search" || name == "x_search") && allowFunctionSearch {
				...
				raw, err := json.Marshal(map[string]string{"type": "x_search"})
				if err != nil {
					return nil, err
				}
		...
		case "web_search", "x_search":
			...
			raw := tool.Raw
			if toolType == "web_search" {
				normalized, err := sjson.Set(tool.Raw, "type", "x_search")
				if err != nil {
					return nil, err
				}
				raw = normalized
	...
	if !hasCompanionTool && !hasSearchIntent {
		return body, nil
	}
	// Only add a cache-routing search tool when the request already contains search
	// intent, unless the account explicitly allows pure client tools. xAI treats
	// web_search and x_search as the same hosted tool and rejects requests containing
	// both, so mixed-tool requests must contain exactly one canonical x_search entry.
	if !allowPureClientTools && !hasSearchIntent {
		return body, nil
	}
	if !searchAdded {
		raw, err := json.Marshal(map[string]string{"type": "x_search"})
