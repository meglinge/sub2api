...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	return s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(ctx), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, nil, false, 0, "")
}
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, windowExcludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) (*Account, error) {
	platform = normalizeOpenAICompatiblePlatform(platform)
	...
	selected, compactBlocked := s.selectBestAccount(ctx, groupID, platform, accounts, requestedModel, excludedIDs, windowExcludedIDs, requireCompact, requiredCapability)
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, windowExcludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability) (*Account, bool) {
	platform = normalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability) (*AccountSelectionResult, error) {
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		...
		if s.concurrencyService != nil {
			if allAccounts, listErr := s.listSchedulableAccounts(ctx, groupID, platform); listErr == nil {
		...
		account, err := s.selectAccountForModelWithExclusions(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, windowExcludedIDs, requireCompact, stickyAccountID, requiredCapability)
		if err != nil && len(yellowWindowIDs) > 0 {
			...
			account, err = s.selectAccountForModelWithExclusions(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, degradedWindowIDs, requireCompact, stickyAccountID, requiredCapability)
		}
		if err != nil {
			return nil, err
		}
	...
	// UsageWindow 分区：优先使用 preferred（Green/Unknown）账号，全部 Red/Yellow 时回退 degraded（Yellow）。
