...
func TestApplyGrokCacheIdentityAppendsNativeToolsWhenSearchPresent(t *testing.T) {
	...
	// Function tools including web_search are normalized to one hosted x_search.
	...
	tools := gjson.GetBytes(body, "tools").Array()
	require.Len(t, tools, 2, "lookup(function) + x_search(native)")
	require.Equal(t, "function", tools[0].Get("type").String())
	require.Equal(t, "lookup", tools[0].Get("name").String())
	require.Equal(t, "x_search", tools[1].Get("type").String())
}
...
func TestApplyGrokCacheIdentityRequiresPatchedFunctionTools(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "native tools only", patchedBody: `{"model":"grok-4.5","tools":[{"type":"x_search"}]}`},
