...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyBusyFallsBackToLoadBalance(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10100)
	accounts := []Account{
		{
	...
	// 上游语义：escape 关闭时，即使账号并发满，仍返回 WaitPlan（保持粘性会话）。
	// Upstream semantics: with escape disabled, a full sticky account returns a WaitPlan
	// (the caller waits for the slot rather than falling back to load balance).
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(21001), selection.Account.ID, "escape disabled: sticky account should be waited on, not abandoned")
