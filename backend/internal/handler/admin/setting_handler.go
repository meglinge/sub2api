...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	openAIUsageWindowDefaults := h.settingService.GetOpenAIUsageWindowConfig(c.Request.Context())
	if req.OpenAIUsageWindowYellow5HPercent <= 0 {
		req.OpenAIUsageWindowYellow5HPercent = previousSettings.OpenAIUsageWindowYellow5HPercent
		if req.OpenAIUsageWindowYellow5HPercent <= 0 {
			req.OpenAIUsageWindowYellow5HPercent = openAIUsageWindowDefaults.Yellow5hPercent
		}
	}
	if req.OpenAIUsageWindowYellow7DPercent <= 0 {
		req.OpenAIUsageWindowYellow7DPercent = previousSettings.OpenAIUsageWindowYellow7DPercent
		if req.OpenAIUsageWindowYellow7DPercent <= 0 {
			req.OpenAIUsageWindowYellow7DPercent = openAIUsageWindowDefaults.Yellow7dPercent
		}
	}
	if req.OpenAIUsageWindowSnapshotStaleSeconds <= 0 {
		req.OpenAIUsageWindowSnapshotStaleSeconds = previousSettings.OpenAIUsageWindowSnapshotStaleSeconds
		if req.OpenAIUsageWindowSnapshotStaleSeconds <= 0 {
			req.OpenAIUsageWindowSnapshotStaleSeconds = int(openAIUsageWindowDefaults.StaleTTL.Seconds())
		}
	}
	...
	if req.OpenAIUsageWindowYellow5HPercent <= 0 || req.OpenAIUsageWindowYellow5HPercent > 100 {
		response.Error(c, http.StatusBadRequest, "openai_usage_window_yellow_5h_percent must be in (0,100]")
		return
	}
	if req.OpenAIUsageWindowYellow7DPercent <= 0 || req.OpenAIUsageWindowYellow7DPercent > 100 {
		response.Error(c, http.StatusBadRequest, "openai_usage_window_yellow_7d_percent must be in (0,100]")
		return
	}
	if req.OpenAIUsageWindowSnapshotStaleSeconds <= 0 {
		response.Error(c, http.StatusBadRequest, "openai_usage_window_snapshot_stale_seconds must be positive")
		return
	}
